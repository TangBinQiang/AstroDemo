import { p as pLimit, C as CONTENT_LAYER_TYPE, L as LIVE_CONTENT_TYPE, d as defineCollection } from './_astro_content_Y-i0Abxi.mjs';
import path, { relative } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { i as commonjsGlobal, j as getDefaultExportFromCjs, k as bold, l as green, z, o as objectType, n as stringType, p as booleanType, q as arrayType, t as dateType } from './astro/server_DbTCsXAD.mjs';
import { s as slash } from './consts_DF5Kf-cg.mjs';
import require$$0, { posix } from 'path';
...
function requireUtils () {
	...
	utils.isRootDirectory = isRootDirectory;
...
function requireWalker () {
	...
	var __importStar = (walker && walker.__importStar) || (function () {
...
function normalizePattern(pattern, expandDirectories, cwd, props, isIgnore) {
	...
	if (require$$0.isAbsolute(result.replace(ESCAPING_BACKSLASHES, ""))) result = posix.relative(escapedCwd, result);
	...
	if (!isIgnore && props.depthOffset >= 0) {
		...
		props.root = newCommonPath.length > 0 ? require$$0.posix.join(cwd, ...newCommonPath) : cwd;
	}
	return result;
}
...
async function glob$1(patternsOrOptions, options) {
	...
	const cwd = opts.cwd ? require$$0.resolve(opts.cwd).replace(BACKSLASHES, "/") : process.cwd().replace(BACKSLASHES, "/");
...
defineCollection({
  ...
  schema: ({ image }) => objectType({
    author: stringType().default(SITE.author),
    pubDatetime: dateType(),
    modDatetime: dateType().optional().nullable(),
    title: stringType(),
    featured: booleanType().optional(),
    draft: booleanType().optional(),
    tags: arrayType(stringType()).default(["others"]),
    ogImage: image().or(stringType()).optional(),
    description: stringType(),
    canonicalURL: stringType().optional(),
    hideEditPost: booleanType().optional(),
